...
public class ShoppingListService
{
    ...
    public async Task<Result<List<ShoppingItemResponse>>> SyncFromCurrentWeeklyPlanAsync(CancellationToken ct)
    {
        ...
        // Purge note entries for weeks that are no longer current or next week.
        // Keeps manually-added items (Notes == null) and sync entries for current/next week only.
        await PurgeStaleWeekNotesAsync(currentWeek, nextWeek, wsId, ct);

        return await GetAllAsync(ct);
    }

    private async Task PurgeStaleWeekNotesAsync(int currentWeek, int nextWeek, Guid wsId, CancellationToken ct)
    {
        string[] keepSentinels = [$" v{currentWeek}d", $" v{nextWeek}d"];

        var itemsWithNotes = await _db.ShoppingItems
        ...
        {
            var parts = item.Notes!.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
            var retained = parts
                .Where(p => !IsSyncNoteEntry(p) || keepSentinels.Any(s => p.Contains(s)))
                .ToList();
            ...
            if (retained.Count == 0)
                ...
                item.Notes = string.Join(", ", retained);
        }

        if (anyChanged) await _db.SaveChangesAsync(ct);
    }

    /// <summary>Returns true when a note part was generated by meal-plan sync (format: "MealName vNdD|guid").</summary>
    private static bool IsSyncNoteEntry(string part) => part.Contains(" v") && part.Contains("d|");
}
